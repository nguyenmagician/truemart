...
$(document).ready(function() {
    $("a.AC").click(function(event) {
        ...
        let email = parent.children(".AC_email").val();
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response.message =="ok") {
                    swal("Good job!", "You have successfully ordered cart", "success");
                } else {
                    swal("Error!", "Something go wrong", "error");
                }
            }
        })
    ...
    $("a.AC_wishlist").click(function(event) {
        let parent = $(event.target).parent().parent(".actions-secondary");
        ...
        let url = location.href.substring(0,location.href.lastIndexOf('/')) + "/inwishlist";
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response.message === "ok") {
                    swal("Added wishlist!", "Item has been in your wishlist", "success");
                } else if (response.message === "already")
                {
                    swal("You like this item much, right", "Item already in your list buddy", "warning");
                }
                else {
    ...
    $("a.AC_compare").click(function(event) {
        let parent = $(event.target).parent().parent(".actions-secondary");

        let product_id = parent.children(".AC_product_id").val();
        let email = parent.children(".AC_email").val();

        let url = location.href.substring(0,location.href.lastIndexOf('/')) + "/incompare";
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response.message =="ok") {
                    swal("Added compare!", "Compared Gotcha!", "success");
                } else if (response.message === "already")
                {
                    swal("Compare duplicated!", "Item already in your compare buddy", "warning");
                }
                else {
                    swal("Error!", response.message || "Something go wrong", "error");
                }
            }
        })
    ...
    $("a.AC_1").click(function(event) {
        let parent = $(event.target).parent("div");

        let product_id = parent.children(".AC_product_id").val();
        let max_quantities = parent.children(".AC_product_sll").val();
        let email = parent.children(".AC_email").val();
        let quantities = $("#AC_value").val();
        let size = $("#sesize").val() || "";


        if(Number(quantities) > Number(max_quantities)) {
            swal("Error!", "Your quantities order should below "+ max_quantities, "error");
        }else {
            let url = location.href.substring(0,location.href.lastIndexOf('/')) + "/incart";
            ...
            $.ajax({
                ...
                success: function (response) {
                    if(response.message =="ok") {
                        swal("Good job!", "You have successfully ordered", "success");
                    } else {
                        swal("Error!", response.message, "error");
                    }
                }
    ...
    $(".max_quantities").keyup(function(event) {

        let quantities = $(event.target).val();
        let max_quantities = $(event.target).attr("max");

        if(Number(quantities) > Number(max_quantities)) {
            swal("Becareful!", "Your order quantities react max stocks", "warning");
            $(event.target).val(max_quantities);
        }
    ...
    $(".wishlist_remove").click(function(event) {
        ...
        let el1 = $(event.target).parents('tr:eq(0)');
        let el2 = $(event.target).parents('tr:eq(0)').find('input.wishlist-amount');

        $(event.target).parents('tr:eq(0)').find('input.wishlist-amount').val('0');
        let value = $(event.target).parents('tr:eq(0)').find('input.wishlist-amount').val();
       ...
       $(event.target).parents('tr:eq(0)').remove();
        ...
        $("form#form-wishlist").submit();
    });

    $("#payment_selectbox").change(function (event) {
        let value = $(this).val();

        if(value === "Indirect Payment") {
            $("#payment_sub_box").hide();
            return
        }

        if(value === "Momo") {
            $("#payment_sub_box").show();
            return
        }
